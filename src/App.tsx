...
function App() {
  ...
  return (
    <>
      <AppHeader mode={mode} theme={theme} onModeChange={switchMode} onToggleTheme={toggleTheme} extra={
              ...
              {signatureLists.map((list, index) => (
                <div key={index} className="glass-panel">
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    {list.signatures.map((sig, sigIndex) => (
...
// ─── Shared App Header ───────────────────────────────────────────────────────
function AppHeader({
  mode, theme, onModeChange, onToggleTheme, extra,
}: {
  mode: AppMode;
  theme: Theme;
    ...
    <header className="app-header">
      <div className="app-header-inner">
        ...
        <div className="app-header-brand">
          <img src="/logo.png" alt="Certificate Manager logo" className="app-header-logo" />
        ...
        <div className="app-header-right">
          {/* NVRAM variable tabs (Secure Boot mode only) */}
          {extra}

          {/* Mode switcher */}
          <div className="mode-switcher">
            <button
              className={`mode-btn${mode === 'secure-boot' ? ' active' : ''}`}
              onClick={() => onModeChange('secure-boot')}
              title="Secure Boot NVRAM Inspector"
            >
              <Shield size={14} />
              Secure Boot
            </button>
            <button
              className={`mode-btn${mode === 'trust-store' ? ' active' : ''}`}
              onClick={() => onModeChange('trust-store')}
              title="Trust Store & File Inspector"
            >
              <FolderOpen size={14} />
              Trust Store
            </button>
